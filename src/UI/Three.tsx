...
const getCenter = (polygon: Polygon): Point => {
    ...
    return [max[0] / 2 + min[0] / 2, max[1] / 2 + min[1] / 2, max[2] / 2 + min[2] / 2];
};
...
const generateSweep = (
    ...
    const difference = [
        polygon[0][0] - minPoint[0],
        polygon[0][1] - minPoint[1],
        polygon[0][2] - minPoint[2],
    ];
    const allPolygons: Polygon[] = [];
    for (let i = 0; i < line.length; i++) {
        const newCenter = [
            line[i][0] + difference[0],
            line[i][1] + difference[1],
            line[i][2] + difference[2],
        ];
        allPolygons.push(
            normalPoly.map(p => [
                p[0] + newCenter[0],
...
class ThreeScene extends Component {
    ...
    addLineToScene = (
        ...
        if (polygon)
            vecPoints.push(new THREE.Vector3(points[0][0], points[0][1], points[0][2]));
