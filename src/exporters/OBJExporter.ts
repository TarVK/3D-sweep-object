...
export class OBJExporter {
    parse(object: Mesh) {
        ...
        function parseMesh(mesh: THREE.Mesh) {
            let nbVertex = 0;
            let nbNormals = 0;
            let nbVertexUvs = 0;
            ...
            if (mesh.material && (mesh.material as THREE.Material).name) {
                output += "usemtl " + (mesh.material as THREE.Material).name + "\n";
            } // vertices
        ...
        function parseLine(line: THREE.Line) {
            let nbVertex = 0;
        ...
        function parsePoints(points: THREE.Points) {
            let nbVertex = 0;
            ...
            if (vertices !== undefined) {
                for (let i = 0, l = vertices.count; i < l; i++, nbVertex++) {
                    ...
                    if (colors !== undefined) {
                        color.fromBufferAttribute(colors as BufferAttribute, i);
