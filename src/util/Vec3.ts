...
export class Vec3 {
     ...
     * @returns The new vector
     */
    public add(vec: Vec3 | Vec4): Vec3;
    public add(x: number | Vec3 | Vec4, y?: number, z?: number): Vec3 {
        if (typeof x == "number") return new Vec3(this.x + x, this.y + y!, this.z + z!);
        else {
     ...
     * @returns The new vector
     */
    public sub(vec: Vec3 | Vec4): Vec3;
    public sub(x: number | Vec3 | Vec4, y?: number, z?: number): Vec3 {
        if (typeof x == "number") return new Vec3(this.x - x, this.y - y!, this.z - z!);
        else {
            if (x instanceof Point3D) x = x.toCartesian();
            return new Vec3(this.x - x.x, this.y - x.y, this.z - x.z);
    ...
    public mul(x: number | Vec3 | Vec4, y?: number, z?: number): Vec3 {
        if (typeof x == "number") {
            if (y == undefined) y = x;
            if (z == undefined) z = x;
     ...
     * @returns The new vector
     */
    public div(vec: Vec3 | Vec4): Vec3;
    public div(x: number | Vec3 | Vec4, y?: number, z?: number): Vec3 {
        if (typeof x == "number") return new Vec3(this.x / x, this.y / y!, this.z / z!);
        else {
            if (x instanceof Point3D) x = x.toCartesian();
            return new Vec3(this.x / x.x, this.y / x.y, this.z / x.z);
